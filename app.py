        ...
        st.session_state.windows = [
            {
                ...
                "OVERALL WIDTH": 1200.0,
                "OVERALL HEIGHT": 1500.0,
                "VENT WIDTH": 600.0,
                "BOTTOM FIXED HEIGHT": 400.0,
...
def add_window(default_variant: str, default_glass_spec: str, variant_map: dict[str, str]):
    ...
    st.session_state.windows.append(
        {
            "id": next_id,
            "label": f"W{len(st.session_state.windows) + 1}",
            "variant_key": default_variant,
            "variant_label": variant_map[default_variant],
            "window_qty": 1,
            "glass_spec": default_glass_spec,
...
with st.sidebar:
    st.header("Brand & Project")
    logo_file = st.file_uploader("Upload logo", type=["png", "jpg", "jpeg"])
    primary = st.color_picker("Primary colour", "#0F4C81")
    accent = st.color_picker("Accent colour", "#16A34A")
    project_name = st.text_input("Project name", "Ali Fabrication Project")
    client_name = st.text_input("Client name", "")
    finish = st.text_input("Aluminium finish", "Powder Coated")

    st.header("Material Controls")
    stock_length_mm = st.number_input("Aluminium stock length (mm)", min_value=1000.0, value=6400.0, step=100.0)
    glass_sheet_width_mm = st.number_input("Glass sheet width (mm)", min_value=500.0, value=3660.0, step=10.0)
    glass_sheet_height_mm = st.number_input("Glass sheet height (mm)", min_value=500.0, value=2440.0, step=10.0)
    kerf_mm = st.number_input("Saw / cut kerf (mm)", min_value=0.0, value=3.0, step=0.5)
    default_row_glass = st.selectbox("Default glass specification", glass_specs, index=max(0, glass_specs.index(default_glass_spec)) if default_glass_spec in glass_specs else 0)

inject_brand_css(primary, accent)

logo_b64 = file_to_base64(logo_file) if logo_file else ""
logo_html = f'<img src="data:image/png;base64,{logo_b64}" style="height:60px; width:auto; margin-bottom:12px;" />' if logo_b64 else ""
...
st.markdown(
    ...
    <div class="brand-hero">
        {logo_html}
        <h1>{project_name}</h1>
        <p>Smart aluminium and glass planning with offcut allocation, ordering totals, optimiser routing, and export-ready jobcards.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
tab1, tab2, tab3 = st.tabs(["Window Entry", "Offcuts", "Results"])
...
with tab1:
    ...
    with action_col1:
        if st.button("Add window"):
            ...
            st.rerun()
    with action_col2:
        st.caption("Each window is edited in its own card so typing stays smooth and stable.")

    for idx, window in enumerate(st.session_state.windows):
        title = f"{window.get('label', f'W{idx+1}')} — {variant_map.get(window.get('variant_key'), 'Typology')}"
        with st.expander(title, expanded=True):
            ...
            with c2:
                variant_key = st.selectbox(
                    "Typology",
                    options=[k for k, _ in variant_options],
                    index=[k for k, _ in variant_options].index(window.get("variant_key", default_variant)),
            ...
            with d1:
                overall_w = st.number_input("Overall Width (mm)", min_value=0.0, value=float(window.get("OVERALL WIDTH", 0.0)), step=10.0, key=f"ow_{window['id']}")
            with d2:
                overall_h = st.number_input("Overall Height (mm)", min_value=0.0, value=float(window.get("OVERALL HEIGHT", 0.0)), step=10.0, key=f"oh_{window['id']}")
            with d3:
                vent_w = st.number_input("Vent Width (mm)", min_value=0.0, value=float(window.get("VENT WIDTH", 0.0)), step=10.0, key=f"vw_{window['id']}")

            e1, e2, e3 = st.columns(3)
            with e1:
                bottom_fixed = st.number_input("Bottom Fixed Height (mm)", min_value=0.0, value=float(window.get("BOTTOM FIXED HEIGHT", 0.0)), step=10.0, key=f"bfh_{window['id']}")
            with e2:
                bottom_clearance = st.number_input("Bottom Clearance (mm)", min_value=0.0, value=float(window.get("BOTTOM CLEARANCE REQUIRED", 0.0)), step=10.0, key=f"bcr_{window['id']}")
            with e3:
                main_vent = st.number_input("Main Vent Width (mm)", min_value=0.0, value=float(window.get("MAIN VENT WIDTH", 0.0)), step=10.0, key=f"mvw_{window['id']}")
            ...
            update_window_field(idx, "OVERALL WIDTH", overall_w)
            update_window_field(idx, "OVERALL HEIGHT", overall_h)
            update_window_field(idx, "VENT WIDTH", vent_w)
            update_window_field(idx, "BOTTOM FIXED HEIGHT", bottom_fixed)
            update_window_field(idx, "BOTTOM CLEARANCE REQUIRED", bottom_clearance)
            update_window_field(idx, "MAIN VENT WIDTH", main_vent)

            if len(st.session_state.windows) > 1 and st.button("Remove this window", key=f"remove_{window['id']}"):
...
# Better order breakdown after offcuts
...
if not bar_df.empty:
    ...
    order_breakdown = pd.DataFrame(columns=["profile", "bars_to_order", "ordered_length_mm", "used_length_mm", "waste_mm", "ordered_length_m", "used_length_m", "waste_m"])
...
with tab3:
    ...
    with r1:
        ...
        st.caption("This is the actual stock to order, based on bars still needed after using available offcuts.")
