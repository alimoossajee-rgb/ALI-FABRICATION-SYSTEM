...
def render_aluminium_bar_layouts(aluminium: dict):
    ...
    for bar in bars:
        ...
        for idx, cut in enumerate(cuts, start=1):
            ...
            degree = str(cut.get("cut_degree", "90")).strip()
            degree_text = f"{degree}°" if degree.isdigit() else degree
            win = cut.get("window_label", "")
            label = f"{win}<br>{int(length)} mm<br>{degree_text}"
            width_pct = max((length / stock) * 100.0, 2.0)
            segs.append(
                f'<div style="width:{width_pct:.3f}%;min-width:54px;height:74px;background:#dbeafe;border-right:1px solid white;display:flex;align-items:center;justify-content:center;text-align:center;font-size:11px;font-weight:700;color:#1e3a8a;padding:4px;overflow:hidden;line-height:1.1;">{label}</div>'
            )
            running += length
            if idx < len(cuts):
                cut_lines.append(f'<span class="mini-tag">Cut line {idx}: {degree_text}</span>')

        if waste > 0:
            waste_pct = max((waste / stock) * 100.0, 2.0)
            segs.append(
                f'<div style="width:{waste_pct:.3f}%;min-width:46px;height:74px;background:#fee2e2;display:flex;align-items:center;justify-content:center;text-align:center;font-size:11px;font-weight:700;color:#991b1b;padding:4px;line-height:1.1;">Waste<br>{int(waste)} mm</div>'
            )

        cut_list_html = ''.join(
            f'<div style="padding:6px 10px;border:1px solid rgba(148,163,184,0.18);border-radius:10px;background:white;font-size:12px;"><b>{i+1}.</b> {c.get("window_label","")} · {int(float(c.get("length_mm",0) or 0))} mm · {str(c.get("cut_degree","90"))}°</div>'
        ...
        st.markdown(
            ...
            <div class="soft-card">
                ...
                <div style="display:flex;width:100%;border-radius:14px;overflow:hidden;border:1px solid rgba(148,163,184,0.25);background:white;margin-bottom:10px;">
                    {''.join(segs)}
                </div>
                <div style="margin-bottom:8px;">{''.join(cut_lines) if cut_lines else '<span class="mini-tag">Single cut piece</span>'}</div>
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:8px;">{cut_list_html}</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
