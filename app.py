...
def render_aluminium_bar_layouts(aluminium: dict):
    ...
    for bar in bars:
        ...
        for cut in cuts:
            ...
            label = f"{cut.get('profile','')}<br>{int(length)} mm"
            width_pct = max((length / stock) * 100.0, 2.0)
            labels.append(label)
            segs.append(
                f'<div style="width:{width_pct:.3f}%;min-width:48px;height:54px;background:#dbeafe;border-right:1px solid white;display:flex;align-items:center;justify-content:center;text-align:center;font-size:12px;font-weight:600;color:#1e3a8a;padding:4px;overflow:hidden;">{label}</div>'
            )
        if waste > 0:
            waste_pct = max((waste / stock) * 100.0, 2.0)
            segs.append(
                f'<div style="width:{waste_pct:.3f}%;min-width:36px;height:54px;background:#fee2e2;display:flex;align-items:center;justify-content:center;text-align:center;font-size:12px;font-weight:600;color:#991b1b;padding:4px;">Waste<br>{int(waste)} mm</div>'
            )
