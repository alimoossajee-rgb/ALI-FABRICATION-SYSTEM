...
def render_aluminium_bar_layouts(aluminium: dict):
    ...
    for bar in bars:
        ...
        if not cuts:
            st.warning("No cut pieces were attached to this bar. This usually means the bar data was created before the cut-piece fix.")
            continue
        ...
        for idx, cut in enumerate(cuts, start=1):
            ...
            segs.append(
                f'<div style="width:{width_pct:.4f}%;min-width:72px;height:90px;background:#dbeafe;'
                ...
                f'{win}<br>{int(length)} mm<br>{angle}</div>'
            )
        ...
        if waste > 0:
            ...
            segs.append(
                f'<div style="width:{waste_pct:.4f}%;min-width:58px;height:90px;background:#fee2e2;'
