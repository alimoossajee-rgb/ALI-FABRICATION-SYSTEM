...
def render_aluminium_bar_layouts(aluminium: dict):
    ...
    for bar in bars:
        ...
        for idx, cut in enumerate(cuts, start=1):
            ...
            segs.append(
                f'<div style="width:{width_pct:.4f}%;min-width:64px;height:86px;'
                'background:#dbeafe;border-right:1px solid white;display:flex;align-items:center;justify-content:center;'
                'text-align:center;font-size:11px;font-weight:700;color:#1e3a8a;padding:4px;overflow:hidden;line-height:1.12;">'
                f'{win}<br>{int(length)} mm<br>{degree_text}</div>'
            )

            cards.append(
                '<div style="padding:7px 10px;border:1px solid rgba(148,163,184,0.18);'
            ...
            running += length
            if idx < len(cuts):
                left_pct = max(min((running / stock) * 100.0, 99.8), 0.2)
                markers.append(
                    f'<div style="position:absolute;left:calc({left_pct:.4f}% - 1px);top:30px;bottom:0;width:2px;background:#0f172a;"></div>'
                    f'<div style="position:absolute;left:calc({left_pct:.4f}% - 26px);top:0;'
                    'background:#0f172a;color:white;border-radius:999px;padding:3px 8px;'
                    f'font-size:11px;font-weight:700;">{degree_text}</div>'
                )

        if waste > 0:
            waste_pct = max((waste / stock) * 100.0, 2.0)
            segs.append(
                f'<div style="width:{waste_pct:.4f}%;min-width:56px;height:86px;'
                'background:#fee2e2;display:flex;align-items:center;justify-content:center;'
                'text-align:center;font-size:11px;font-weight:700;color:#991b1b;padding:4px;line-height:1.1;">'
                f'Waste<br>{int(waste)} mm</div>'
            )

        if not cards:
            cards_html = '<div class="mini-tag">No cut pieces on this bar</div>'
        else:
