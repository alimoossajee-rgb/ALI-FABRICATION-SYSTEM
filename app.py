...
def preview_base64(variant_key: str) -> str:
    p = PREVIEW_DIR / f"{safe_name(variant_key)}.png"
    if p.exists():
...
with st.sidebar:
    ...
    accent = st.color_picker("Accent colour", "#F28C36")
...
with tab1:
    ...
    with a1:
        if st.button("Add window"):
            ...
            st.rerun()
    with a2:
        st.caption("Each card shows the typology image from your calculator sheet and the matching system code.")
    ...
    for idx, window in enumerate(st.session_state.windows):
        ...
        with st.expander(title, expanded=True):
            ...
            with preview_col:
                ...
                variant_short = selected_variant_label.split("·", 1)[1].strip() if "·" in selected_variant_label else selected_variant_label
